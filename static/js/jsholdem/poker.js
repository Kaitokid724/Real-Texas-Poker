...
{
  var html = "<html><body topmargin=2 bottommargin=0 bgcolor=" + BG_HILITE + " onload='document.f.y.focus();'>" +
  "<font>" + text + "</font><form name=f><input name=y type=button value='  OK  ' " +
...
function handle_how_many_reply (opponents) 
{
  ...
  gui_initialize_css();         // cargar imagenes de fondo
...
{
  count = count + 1; // contar al jugador
...
{
  var my_players = [
  new player("Mili", 0, "", "", "", 0, 0),
  new player("Richard", 0, "", "", "", 0, 0),
  new player("Rocket", 0, "", "", "", 0, 0),
  new player("Marta", 0, "", "", "", 0, 0),
  new player("Sara", 0, "", "", "", 0, 0),
  new player("Elkin", 0, "", "", "", 0, 0),
  new player("Zato", 0, "", "", "", 0, 0),
  new player("Ana", 0, "", "", "", 0, 0),
  new player("Rosario", 0, "", "", "", 0, 0)];
...
function deal_and_write_a () 
{
  ...
  // Repartir cartas solo a los que aun quedan activos
  do {
...
function deal_flop () 
{
  ...
  // Repartir las 3 primeras cartas 
...
function main () 
{
  ...
  if (players[current_bettor_index].status == "BUST" ||
    ...
    if (current_bettor_index == 0) 
    {
      ...
      {
        if (quick_values[i]) {
          quick_bets += "<a href='javascript:parent.handle_human_bet(" + quick_values[i] + ")' class= 'grad"+ i +"'>" + quick_values[i] + "</a>" + "";
        }
      }
      quick_bets += "<a href='javascript:parent.handle_human_bet(" + players[0].bankroll + ")' class='grad6'>All In!</a>";
      var html9 = "<td><table align=center><tr><td align=center>";
      ...
      var message = "<b>Current raise: " + current_bet_amount + "</b>&nbsp;&nbsp;You need <font color=" + hi_lite_color + " style=''><b>" + to_call + "</b></font> more to call.&nbsp;&nbsp;";
      gui_write_game_response(message);
...
function handle_end_of_round () 
{
  ...
  {
    // No eliminar en primera ronda a quin no entre?
  ...
  for (i = 0; i < players.length; i++) {
    if (players[i].total_bet == 0 && players[i].status == "BUST") {
      continue;  // Saltarse a los que se retiran
    }
  ...
  var hi_lite_color = gui_get_theme_mode_highlite_color();
  var html = "<html><body bgcolor=" + BG_HILITE + " onload='document.f.c.focus();'>" + get_pot_size_html() + "&nbsp;&nbsp;<font color=" + hi_lite_color + "><b>Winning: " + winner_text + "</b></font>" + detail + "&nbsp;&nbsp;";
...
function the_bet_function (player_index, bet_amount) 
{
  ...
  } else if (bet_amount >= players[player_index].bankroll) 
  {
  ...
  } else if (bet_amount + players[player_index].subtotal_bet == current_bet_amount) 
    {
      ...
      if (player_index == 9)
        document.getElementById("bbet9").style.background = "lawngreen";
    } else if (current_bet_amount > players[player_index].subtotal_bet + bet_amount) 
      { 
               if (player_index == 0)
               document.getElementById("mybet").style.background = "lawngreen";
               if (player_index == 1)
